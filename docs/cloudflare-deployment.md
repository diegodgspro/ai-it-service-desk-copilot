...
## v1.4.0 delivery B: local recovery and quotas

Delivery B remains local/CI-only. Use `npm run test:recovery` for the disposable two-run rehearsal and consult [recovery and retention](recovery-retention.md). Official quota consumption must be checked in the Cloudflare dashboard; application counts are advisory and do not authorize billing, migration, deployment, export or restore.
